...
const AddProductPage = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // 1. Creează o referință unică pentru imagine în Firebase Storage
      const storageRef = ref(storage, `products/${Date.now()}_${imageFile.name}`);
      
      // 2. Încarcă fișierul folosind 'uploadBytes'
      await uploadBytes(storageRef, imageFile);
      
      // 3. Obține URL-ul de download al imaginii
      const downloadURL = await getDownloadURL(storageRef);

      // 4. Salvează datele produsului (inclusiv URL-ul imaginii) în Firestore
      const newProduct = {
        name: formState.name,
