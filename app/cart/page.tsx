  ...
  return (
    <div className="py-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-extrabold text-white mb-8">Coșul tău de cumpărături</h1>

        {!isClient || isStockLoading ? (
          <div className="text-center bg-gray-800 p-8 rounded-lg shadow-md">
            <p className="text-gray-400 text-lg">Se încarcă coșul...</p>
          </div>
        ) : cartItems.length === 0 ? (
          <div className="text-center bg-gray-800 p-8 rounded-lg shadow-md">
            <p className="text-gray-400 text-lg">Coșul tău este gol.</p>
            <Link href="/" className="mt-4 inline-block px-6 py-3 font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700">
          ...
          <div className="bg-gray-800 p-8 rounded-lg shadow-md">
              ...
              {cartItems.map(item => {
                ...
                return (
                  <li key={item.id} className="flex py-6 items-center">
                    <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-700 bg-gray-700"></div>
                    <div className="ml-4 flex flex-1 flex-col">
                      ...
                      <div className="flex flex-1 items-end justify-between text-sm mt-4">
                        <div className="flex items-center border border-gray-600 rounded-md bg-gray-700">
                          <button onClick={() => updateItemQuantity(item.id, item.quantity - 1)} className="px-2 py-1 text-gray-300 hover:bg-gray-600 rounded-l-md" disabled={item.quantity <= 1}>
                            <Minus size={16} />
                          </button>
                          <input 
                            type="number"
                            ...
                            className="w-12 text-center border-y-0 border-x border-gray-600 bg-gray-700 text-white font-semibold focus:outline-none focus:ring-0"
                           />
                          <button 
                            onClick={() => updateItemQuantity(item.id, item.quantity + 1)} 
                            className="px-2 py-1 text-gray-300 hover:bg-gray-600 rounded-r-md" 
                        ...
                        <div className="flex">
                          <button onClick={() => removeFromCart(item.id)} type="button" className="font-medium text-red-500 hover:text-red-400 flex items-center gap-1">
                            <Trash2 size={16} />
                            <span>Șterge</span>
                          </button>
                        </div>
            ...
            <div className="border-t border-gray-700 pt-6 mt-6">
              ...
              <p className="mt-1 text-sm text-gray-400">Taxele de livrare vor fi calculate la checkout.</p>
              <div className="mt-6">
